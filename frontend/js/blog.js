...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  if (dashboardName) {
    dashboardName.textContent = currentUser?.name || currentUser?.email || 'User';
  }
  ...
  if (createForm) {
    createForm.addEventListener('submit', async function (e) {
      ...
      if (!title) {
        ...
        isValid = false;
      } else hideError('blogTitleError');

      if (content.length < 10) {
        showError('blogContentError', 'Content should be at least 10 characters.');
        isValid = false;
      } else hideError('blogContentError');
      ...
      try {
        ...
        showAlert('createBlogAlert', 'Blog post published! Redirecting to dashboard...', 'success');
        createForm.reset();
...
async function renderDashboardPosts(postList) {
  postList.innerHTML = '<p class="empty-state">Loading your posts...</p>';
  ...
  try {
    ...
    if (posts.length === 0) {
      postList.innerHTML = '<p class="empty-state">No blog posts yet. Click "Create Blog" to write your first one!</p>';
      return;
    }
    ...
    posts.slice().reverse().forEach(function (post) {
      ...
      row.innerHTML =
        ...
        '<p class="post-excerpt">' + content.slice(0, 100) + (content.length > 100 ? '...' : '') + '</p>' +
        '</div>';
